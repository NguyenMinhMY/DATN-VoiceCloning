...
from src.utility.tokenizer import ArticulatoryCombinedTextFrontend
...
class Aligner(torch.nn.Module):
    def __init__(self, n_mels=80, num_symbols=145, lstm_dim=512, conv_dim=512):
        super().__init__()
    ...
    def label_speech(self, speech):
        ...
        probabilities_of_phones_over_frames = self(speech.unsqueeze(0)).squeeze()[
            :, :73
        ]
        ...
        for index, _ in enumerate(probabilities_of_phones_over_frames):
            ...
            access_safe_next_index = min(
                index + 1, len(probabilities_of_phones_over_frames) - 1
            )
            smoothed_probs = (
                probabilities_of_phones_over_frames[access_safe_prev_index]
                + probabilities_of_phones_over_frames[access_safe_next_index]
                + probabilities_of_phones_over_frames[index]
            ) / 3
            smoothed_phone_probs_over_frames.append(smoothed_probs.unsqueeze(0))
        print(torch.cat(smoothed_phone_probs_over_frames))
        _, phone_ids_over_frames = torch.max(
            torch.cat(smoothed_phone_probs_over_frames), dim=1
        )
    ...
    def inference(
        ...
        if pathfinding == "MAS":
            ...
            dist_matrix, predecessors, *_ = dijkstra(
            ...
            if save_img_for_debug is not None:
                ...
                ax[0].imshow(
                    pred_max, interpolation="nearest", aspect="auto", origin="lower"
                )
                ax[1].imshow(
                    path_plot,
                    interpolation="nearest",
