...
class VastSpeech(torch.nn.Module, ABC):
    """
    VastSpeech module.

    This is a module of VastSpeech 2 inspired from FastSpeech 2: Fast and
    ...
    def __init__(
        ...
        self.proj_mean = torch.nn.Conv1d(adim, adim, 1)
        self.proj_std = torch.nn.Conv1d(adim, adim, 1)
    ...
    def _forward(
        ...
        encoded_texts_mean = self.proj_mean(encoded_texts.transpose(1, 2)).transpose(
            1, 2
        ) * text_masks.transpose(1, 2)
        encoded_texts_std = self.proj_std(encoded_texts.transpose(1, 2)).transpose(
            1, 2
        ) * text_masks.transpose(1, 2)
        ...
        if is_inference:
            ...
            encoded_texts_mean = (
                encoded_texts_mean + embedded_energy_curve + embedded_pitch_curve
            )
            encoded_texts = encoded_texts_mean + torch.exp(
                encoded_texts_std
            ) * torch.randn(encoded_texts_std.size(), device=encoded_texts_std.device)
            ...
            encoded_texts = self.length_regulator(
                encoded_texts, gold_durations, alpha
            )  # (B, Lmax, adim)
