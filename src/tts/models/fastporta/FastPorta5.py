...
class FastPorta5(torch.nn.Module, ABC):
    def __init__(
        self,
        ...
        # speaker embedding
        self.spk_embed_proj = torch.nn.Linear(utt_embed_dim, adim, bias=True)
    ...
    def _forward(
        ...
        utterance_embedding = self.spk_embed_proj(utterance_embedding).unsqueeze(1)
        ...
        encoded_texts = encoded_texts + utterance_embedding
        ...
        if speech_lens is not None and not is_inference:
            ...
            h_masks = None
        zs, _ = self.decoder(encoded_texts, h_masks, None)  # (B, Lmax, adim)
        ...
        if is_inference:
            mel_outs = self.post_flow(
                ...
                infer=is_inference,
                mel_out=mel_outs,
                encoded_texts=utterance_embedding.repeat(1, mel_outs.size(1), 1),
                tgt_nonpadding=None,
            )
        else:
