...
def train_loop(
    ...
    for step_counter in range(step_counter + 1, steps + 1):
        ...
        net.on_train_start(step_counter - 1)
        ...
        for batch in tqdm(train_loader):
            with autocast(enabled=enable_autocast, cache_enabled=False):
                ...
                (loss_dict, outputs) = net(
                    text_tensors=batch[0].to(device),
                    text_lens=batch[1].to(device),
                    gold_speech=batch[2].to(device),
                    speech_lens=batch[3].to(device),
                ...
                (
                    ...
                    batch_of_spectrograms=outputs["model_outputs"],
                    batch_of_spectrogram_lengths=batch[3].to(device),
                    return_all_outs=True,
                )
                ...
                for key, value in loss_dict.items():
                    loss_logs[key].append(value.item())
                loss_logs["loss_cycle"].append(cycle_dist.item())
                ...
                if step_counter <= phase_1_steps:
                    ...
                    if not cycle_dist.isnan():
                        loss_dict["loss"] = loss_dict["loss"] + cycle_dist
            ...
            clip_grad_norm_(net.parameters(), 5.0, error_if_nonfinite=False)
        ...
        for key, value in loss_logs.items():
            epoch_loss[key] = (
                sum(loss_logs[key]) / len(loss_logs[key])
                if len(loss_logs[key]) > 0
                else 0.0
            )
        ...
        if step_counter % steps_per_save == 0 and step_counter != 1:
            ...
            if epoch_loss["loss"] < best_train_loss:
                best_train_loss = epoch_loss["loss"]
                torch.save(
                    {
            ...
            if (
                epoch_loss["loss_cycle"] < best_cycle_loss
                and epoch_loss["loss_cycle"] != 0.0
            ):
                best_cycle_loss = epoch_loss["loss_cycle"]
                torch.save(
                    {
                        "model": net.state_dict(),
                        "optimizer": optimizer.state_dict(),
        ...
        print(f"\nSteps: {step_counter}")
        print(
            " - ".join([f"{k}: {epoch_loss[k]:.3f}" for k in sorted(epoch_loss.keys())])
        )
