...
from spk_embedding.StyleEmbedding import StyleEmbedding
from utility.warmup_scheduler import WarmupScheduler
...
def train_loop(
    ...
    optimizer = torch.optim.RAdam(
        net.parameters(), lr=lr, eps=1.0e-06, weight_decay=0.0
    )
    ...
    for step in tqdm(range(step_counter, steps)):
        ...
        for batch in batches:
            with autocast():
                if step <= phase_1_steps:
                    ...
                    _train_loss, output_spectrograms = net(
                        text_tensors=batch[0].to(device),
                        text_lengths=batch[1].to(device),
                        gold_speech=batch[2].to(device),
                        speech_lengths=batch[3].to(device),
        ...
        if step % steps_per_checkpoint == 0 and step != 0:
            ...
            print(f"\nTotal Steps: {step}")
            print(
                f"Spectrogram Loss: {round(sum(train_losses_total) / len(train_losses_total), 3)}"
            )
            ...
            if use_wandb:
                wandb.log(
                    {
                        "spectrogram_loss": round(
                            sum(train_losses_total) / len(train_losses_total), 3
                        ),
                        "cycle_loss": round(
                            sum(cycle_losses_total) / len(cycle_losses_total), 3
                        )
                        if len(cycle_losses_total) != 0
                        else 0.0,
                        "Steps": step,
